...
export default function CreateCreditNote(props) {
...
axios.get(process.env.REACT_APP_LOCAL_URL+"/getDocMaster/CreditNote",{//change //debit note change
  headers:{
    "Content-Type":"application/json",
    "Authorization":'Bearer '+token
  }
    ...
    axios.get(process.env.REACT_APP_LOCAL_URL+`/creditNote/year/`+document.querySelector("#financialYear").value,{//change//debit note change
      headers:{
        "Content-Type":"application/json",
        "Authorization":'Bearer '+token
      }
...
const onDescriptionChange=(e)=>{
  ...
  const saveInvoice=(e)=>{
    ...
    axios.post(`${process.env.REACT_APP_LOCAL_URL}/${invoiceType==process.env.REACT_APP_CASH_SALE_INVOICE?"saveCashInvoice":invoiceType==process.env.REACT_APP_PROFORMA_INVOICE?"saveProformaInvoice":"saveCreditNote"}`, invoiceData,{//save invoice //change//debit note change
      headers:{
        "Content-Type":"application/json",
        "Authorization":'Bearer '+token
      }
    ...
    .then(function (response) {
      ...
      setTimeout(()=>{
        ...
        window.location.href = '/creditNoteList?month='+month; 
      },2000)
   ...
   if(editref.current){
     ...
     if(actionedit == "Edit" || actionedit == "Clone"){
       ...
       document.querySelector("#prodtable").innerHTML=''
            ...
            axios.get(`${process.env.REACT_APP_LOCAL_URL}/viewCreditNote?invId=${invNoEdit}`,{//change//debit note change
              headers:{
                "Content-Type":"application/json",
                "Authorization":'Bearer '+token
              }
                ...
                <ul className="breadcrumb invoices-breadcrumb">
                  <li className="breadcrumb-item invoices-breadcrumb-item">
                    <a href="invoices.html">
                      <i className="fa fa-chevron-left"></i> Back to Credit note
                      List
                    </a>
                  </li>
                </ul>
