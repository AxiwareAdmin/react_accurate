...
const exportToExcel = async () => {
    ...
    useEffect(() => {
		...
		var url=new URL(window.location.href);
		  ...
		  let cgst = 0;
		  let sgst = 0;
        axios.post(`${process.env.REACT_APP_LOCAL_URL}/creditNote/${month1}`,{financialYear:localStorage.getItem("financialYear")},header).then((res) => {//debit note change
	...
	// },3000)	
	  ...
	  function rendercommon(e,name , invt) {
		 ...
		 if(name == "Edit"){
			...
			if(res!=null && res.data.res=='sucess'){
				...
				Swal.fire(
					'',
					'Credit note deleted successfully!!',//debit note change
					'success'
				  )
				...
				//   alert("There is some issue delete invoice.");	
				  Swal.fire({
					icon: 'error',
					title: 'Oops...',
					text: 'There is some issue delete credit note.',//debit note change
					footer: ''
				  })	   
		    });
		 } else if(name == "Cancel"){
			...
			// axios.post("http://localhost:8080/cancelInvoice/"+invt,{},header).then((res)=>{
                debugger;
			axios.get(`${process.env.REACT_APP_LOCAL_URL}/cancelCreditNote?QuoId=${invt}`,header).then((res)=>{//debit note change
				if(res!=null && res.data.res=='success'){
					Swal.fire(
						'',
						'Credit note cancelled successfully!!',//debit note change
						'success'
					  )	
					...
					Swal.fire({
						icon: 'error',
						title: 'Oops...',
						text: 'There is some issue to cancel credit note',//debit note change
						footer: ''
					  })	
				}
			}).catch((e)=>{
	  ...
	  function refreshDataTable(data){
		...
		var url=new URL(window.location.href);
		  ...
		  let cgst = 0;
		  let sgst = 0;
        axios.post(`${process.env.REACT_APP_LOCAL_URL}0/getFilterInvoices`,data,{
			...header
		}).then((res) => {
			setInvoicedo(res.data);
