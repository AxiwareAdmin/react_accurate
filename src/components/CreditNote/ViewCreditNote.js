...
export default function ViewCreditNote() {
  ...
  useEffect(() => {
    ...
    if (!initilized.current) {
      ...
      axios
        .get(`${process.env.REACT_APP_LOCAL_URL}/viewCreditNote?invId=` + invId, header)//debit note change
        .then((res) => {
  ...
  const sendMail=  (invoiceNo,custName) => {
    html2canvas(invoicepdf.current).then((canvas) => {
      ...
      axios.post(`${process.env.REACT_APP_LOCAL_URL}/sendmailCreditNote`, formData ,{//debit note change
        headers: {
          Authorization: "Bearer " + token,
        },
      })
        ...
        .catch(error => {
          ...
          Swal.fire({
            icon: 'error',
            title: 'Oops...',
            text: 'There is some issue to mail credit note',//debit note change
            footer: ''
            })
        });
    });
