...
export default function CreateCustomerPo(props) {
...
axios.get(process.env.REACT_APP_LOCAL_URL+"/getDocMaster/Purchase Order",{//change
  headers:{
    ...
    axios.get(process.env.REACT_APP_LOCAL_URL+`/customerPo/year/`+document.querySelector("#financialYear").value,{//change
      headers:{
        "Content-Type":"application/json",
        "Authorization":'Bearer '+token
      }
...
const onDescriptionChange=(e)=>{
  ...
  const saveInvoice=(e)=>{
    ...
    axios.post(`${process.env.REACT_APP_LOCAL_URL}/${invoiceType==process.env.REACT_APP_CASH_SALE_INVOICE?"saveCashInvoice":invoiceType==process.env.REACT_APP_PROFORMA_INVOICE?"saveProformaInvoice":"saveCustomerPo"}`, invoiceData,{//save invoice //change
      headers:{
        "Content-Type":"application/json",
        "Authorization":'Bearer '+token
      }
    ...
    .then(function (response) {
      ...
      setTimeout(()=>{
        ...
        window.location.href = '/CustomerPoList?month='+month; 
      },2000)
   ...
   if(editref.current){
     ...
     if(actionedit == "Edit" || actionedit == "Clone"){
       ...
       document.querySelector("#prodtable").innerHTML=''
            ...
            axios.get(`${process.env.REACT_APP_LOCAL_URL}/viewCustomerPo?invId=${invNoEdit}`,{//change
              headers:{
                "Content-Type":"application/json",
                "Authorization":'Bearer '+token
              }
                ...
                <ul className="breadcrumb invoices-breadcrumb">
                  <li className="breadcrumb-item invoices-breadcrumb-item">
                    <a href="invoices.html">
                      <i className="fa fa-chevron-left"></i> Back to Purchase Order
                      List
                    </a>
                  </li>
                </ul>
