...
export default function CreateCustomerPo(props) {
  ...
  useEffect(() => {
    ...
    axios
      .get(BACKEND_SERVER + "/getDocMaster/CustomerPo", {
        //change
        headers: {
          "Content-Type": "application/json",
          Authorization: "Bearer " + token,
    ...
    axios
      ...
      .then(function (response) {
        ...
        if (response != null && response.data.res == "success") {
          ...
          Swal.fire(
            "",
            "Customer PO created successfully!", //change
            "success"
          );
          ...
          e.target.style.opacity = "";
          alert(
            "There is some issue in creating Customer PO. kindly check whether all the data is entered or not."
          ); //change
        }
      })
      .catch(function (error) {
        ...
        if (response != null && response.data.res == "success") {
          ...
          axios
            ...
            .then((res) => {
              ...
              window.location.href = "/viewCustomerPo?invNo=" + res.data; //change
            });
          setIsSaved(1);
        } else {
          e.target.style.pointerEvents = ""; // Re-enable pointer events
          ...
          Swal.fire(
            "",
            "There is some issue in creating Customer PO.", //change
            "error"
          );
        }
      })
                  ...
                  <ul className="breadcrumb invoices-breadcrumb">
                    <li className="breadcrumb-item invoices-breadcrumb-item">
                      <a href="invoices.html">
                        <i className="fa fa-chevron-left"></i> Back to Customer PO
                        List
                      </a>
                    </li>
                  </ul>
