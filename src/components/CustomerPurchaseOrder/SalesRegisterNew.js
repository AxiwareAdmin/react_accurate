...
export default function SalesRegisterCustomerPO () {
  ...
  const navigate=useNavigate();
    ...
    useEffect(() =>{
      ...
      useEffect(() =>{
        ...
        axios.post(`${process.env.REACT_APP_LOCAL_URL}/viewSalesRegCustomerPo`,{financialYear:localStorage.getItem("financialYear")},header).then((res) =>{//change
        ...
        res.data && res.data.map(ele => {
            ...
            let obj = JSON.parse(ele);
              ...
              trEle.addEventListener('click',()=>{
                ...
                window.location.href=`/CustomerPoList?month=${obj.month}`;//change
              })
              let tdEle = document.createElement("td");
              let aEle = document.createElement("a");
              aEle.className="text-decoration-none";
              aEle.href = `/CustomerPoList?month=${obj.month}`;//change
                        ...
                        <div class="col text-end">
                            <ul class="list-inline-item ps-0">
                                ...
                                <li class="list-inline-item">
                                    <a class="add btn btn-gradient-primary font-weight-bold text-white todo-list-add-btn btn-rounded" href={`/CreateCustomerPo`}>Create Customer PO</a>
                                </li>
                            </ul>
                        </div>
                    </div>
                                ...
                                <table class="table table-striped table-nowrap custom-table mb-0 " style={{width:"100%"}}>
                                    <thead>
                                        <tr  style={{background: "linear-gradient(90deg, rgba(67,203,255,1) 25%, rgba(151,8,204,1) 100%)"}} >
                                            ...
                                            <th style={{color:"#fff",fontWeight:"bolder"}}>Total Customer PO</th>
