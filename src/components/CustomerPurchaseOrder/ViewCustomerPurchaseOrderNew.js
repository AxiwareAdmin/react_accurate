...
export default function ViewCustomerPurchaseOrder() {
  ...
  useEffect(() => {
    ...
    if (!initilized.current) {
      ...
      axios
        .get(`${process.env.REACT_APP_LOCAL_URL}/${invoiceType==process.env.REACT_APP_CASH_SALE_INVOICE?'viewCashInvoice':invoiceType==process.env.REACT_APP_PROFORMA_INVOICE?'viewProformaInvoice':'viewCustomerPo'}?invId=` + invId, header)
        .then((res) => {
          debugger;
                    ...
                    <div class="row">
                      ...
                      >
                        <div class="invoice-item-box">
                          <p>Customer PO No. : {invNo}</p>
                          <p class="mb-0">Customer PO Date : {invoiceDate}</p>
                        </div>
