...
export default function CreateDebitNote(props) {
...
axios.get(process.env.REACT_APP_LOCAL_URL+"/getDocMaster/DebitNote",{//change //debitnotechange
  headers:{
    "Content-Type":"application/json",
    "Authorization":'Bearer '+token
  }
    ...
    axios.get(process.env.REACT_APP_LOCAL_URL+`/debitNote/year/`+document.querySelector("#financialYear").value,{//change//debit note change
      headers:{
        "Content-Type":"application/json",
        "Authorization":'Bearer '+token
      }
  ...
  const [loadingCompleted,setLoadingCompleted]=useState(null)
...
const onDescriptionChange=(e)=>{
  ...
  const saveInvoice=(e)=>{
    ...
    axios.post(`${process.env.REACT_APP_LOCAL_URL}/${invoiceType==process.env.REACT_APP_CASH_SALE_INVOICE?"saveCashInvoice":invoiceType==process.env.REACT_APP_PROFORMA_INVOICE?"saveProformaInvoice":"saveDebitNote"}`, invoiceData,{//save invoice //change//debit note change
      headers:{
        "Content-Type":"application/json",
        "Authorization":'Bearer '+token
      }
    ...
    .then(function (response) {
      ...
      setTimeout(()=>{
        ...
        alert("There is some issue in creating inventory. kindly check whether all the data is entered or not.")
    })
    .catch(function (error) {
      console.log(error);
    });
  ...
  const printButtonClicked=(e)=>{
    ...
    window.location.href="/viewDebitNote?id="+invoiceNumber;//change//debit note change
  }
   ...
   if(editref.current){
     ...
     if(actionedit == "Edit" || actionedit == "Clone"){
       ...
       document.querySelector("#prodtable").innerHTML=''
            ...
            axios.get(`${process.env.REACT_APP_LOCAL_URL}/viewDebitNote?invId=${invNoEdit}`,{//change//debit note change
              headers:{
                "Content-Type":"application/json",
                "Authorization":'Bearer '+token
              }
                ...
                <ul className="breadcrumb invoices-breadcrumb">
                  <li className="breadcrumb-item invoices-breadcrumb-item">
                    <a href="invoices.html">
                      <i className="fa fa-chevron-left"></i> Back to Debit note
                      List
                    </a>
                  </li>
                </ul>
