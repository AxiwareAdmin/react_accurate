...
const exportToExcel = async () => {
    ...
    useEffect(() => {
		...
		var url=new URL(window.location.href);
		  ...
		  let cgst = 0;
		  let sgst = 0;
        axios.post(`${process.env.REACT_APP_LOCAL_URL}/debitNote/${month1}`,{financialYear:localStorage.getItem("financialYear")},header).then((res) => {//debit note change
	  ...
	  function rendercommon(e,name , invt) {
		 ...
		 if(name == "Edit"){
			...
			if(res!=null && res.data.res=='sucess'){
				...
				Swal.fire(
					'',
					'Debit note deleted successfully!!',//debit note change
					'success'
				  )
				...
				//   alert("There is some issue delete invoice.");	
				  Swal.fire({
					icon: 'error',
					title: 'Oops...',
					text: 'There is some issue delete debit note.',//debit note change
					footer: ''
				  })	   
		    });
		 } else if(name == "Cancel"){
			...
			// axios.post("http://localhost:8080/cancelInvoice/"+invt,{},header).then((res)=>{
                debugger;
			axios.get(`${process.env.REACT_APP_LOCAL_URL}/cancelDebitNote?QuoId=${invt}`,header).then((res)=>{//debit note change
				if(res!=null && res.data.res=='success'){
					Swal.fire(
						'',
						'Debit note cancelled successfully!!',//debit note change
						'success'
					  )	
					...
					Swal.fire({
						icon: 'error',
						title: 'Oops...',
						text: 'There is some issue to cancel debit note',//debit note change
						footer: ''
					  })	
				}
			}).catch((e)=>{
