...
const exportToExcel = async () => {
	...
	useEffect(()=>{
	   ...
	   setCanInvVal(caninvvals);
		...
		aElem.addEventListener('click',()=>{
			...
			window.location.href=`/ViewDebitNoteTriplet?id=${elem.invoiceId}`;//change
		})
    ...
    useEffect(() => {
		...
		var url=new URL(window.location.href);
		  ...
		  let cgst = 0;
		  let sgst = 0;
        axios.post(`${process.env.REACT_APP_LOCAL_URL}/${invoiceType==process.env.REACT_APP_CASH_SALE_INVOICE?"cashInvoices":invoiceType==process.env.REACT_APP_PROFORMA_INVOICE?"proformaInvoices":"debitNote"}/${month1}`,{financialYear:localStorage.getItem("financialYear")},header).then((res) => {//change
		...
		for(let i=0;i<lFootTrList.length;i++){
		   ...
		   setCanInvVal(caninvvals);
			...
			aElem.addEventListener('click',()=>{
                ...
                window.location.href=`/ViewDebitNoteTriplet?id=${elem.invoiceId}`;
            })
            aElem.appendChild(textElem); 
			tdElem.appendChild(aElem);
            trElem.appendChild(tdElem);
	...
	// },3000)	
	  ...
	  function rendercommon(e,name , invt) {
		 ...
		 if(name == "Edit"){
			...
			window.location.href=`/CreateDebitNote?InvNo=${invt}&action=Edit`;//change
		...
		//  }
			...
			window.location.href=`/ViewDebitNote?invNo=${invt}`;
		 }
		 else if(name == "Print"){
			// navigate("/viewInvoiceTriplet?id="+invt,{state:{invoiceType:'GST'}});

			window.location.href=`/ViewDebitNoteTriplet?id=${invt}`;
		 }
		 
		 else if(name == "Delete"){
			axios.get(`${process.env.REACT_APP_LOCAL_URL}/deleteDebitNote?QuoId=${invt}`,header).then((res) => {//change
		    console.log(res.data);
			if(res!=null && res.data.res=='sucess'){
				...
				//   alert("There is some issue delete invoice.");	
				  Swal.fire({
					icon: 'error',
					title: 'Oops...',
					text: 'There is some issue delete Debit Note.',
					footer: ''
				  })	   
		    });
		 } else if(name == "Cancel"){
			...
			axios.get(`${process.env.REACT_APP_LOCAL_URL}/cancelDebitNote?QuoId=${invt}`,header).then((res)=>{//change
				if(res!=null && res.data.res=='success'){
					...
					Swal.fire({
						icon: 'error',
						title: 'Oops...',
						text: 'There is some issue to cancel Debit Note',
						footer: ''
					  })	
				}
			}).catch((e)=>{
				Swal.fire({
					...
					// alert(custname)
					window.location.href=`/ViewDebitNote?invNo=${invt}&custName=${custname}&action=send`;

					
				
		 } else if(name == "Copy"){
			window.location.href=`/CreateDebitNote?InvNo=${invt}&action=Clone`;
        ...
        'success'
			...
			window.location.href=`/ViewDebitNote?invNo=${invt}&action=download`;
        ...
        axios.post(`${process.env.REACT_APP_LOCAL_URL}/getFilterInvoices`,data,{
          ...
          //  tdElem.appendChild(labelElem);
			...
			aElem.addEventListener('click',()=>{
                ...
                window.location.href=`/ViewDebitNoteTriplet?id=${elem.invoiceId}&${process.env.REACT_APP_INVOICE_TYPE}=${invoiceType}`;
            })
		    // aElem.href="/viewInvoiceTriplet?id="+elem.invoiceId;
            aElem.appendChild(textElem); 
			tdElem.appendChild(aElem);
                ...
                <div class="content container-fluid">
                	...
                	<div class="crms-title row bg-white">
                		<div class="col  p-0">
			                ...
			                </span>{invoiceType==process.env.REACT_APP_PROFORMA_INVOICE?"Proforma":""} Debit Note </h3>
                		</div>
                        <div class="col p-0 text-end">
                			<ul class="breadcrumb bg-white float-end m-0 ps-0 pe-0">
								<li class="breadcrumb-item"><a href="index.html">Dashboard</a></li>
								<li class="breadcrumb-item active">{invoiceType==process.env.REACT_APP_PROFORMA_INVOICE?"Proforma":""} Debit Note</li>
							</ul>
                		</div>
                    </div>
								...
								<div class="row align-items-center">
									<div class="col-lg-8 col-md-8">
										<div class="invoices-tabs">
											<ul>
												...
												<li><a href="#" class="active">All Debit Note</a></li>
									...
									<div class="col-lg-4 col-md-4">
										<div class="invoices-settings-btn">
											...
											<a href="/CreateDebitNote" class="btn">
												<i data-feather="plus-circle"></i> New Debit Note
											</a>
										</div>
									</div>
								</div>
						...
						<div class="col-xl-3 col-sm-6 col-12 ">
							<div class="card inovices-card" style={{height:'90%'}}>
								<div class="card-body">
									...
									<p class="inovices-all" style={{fontSize:'18px',display:'flex',justifyContent:'space-evenly',alignItems:'end',flexWrap:'wrap'}}>All Debit Note <div style={{fontSize:'15px'}}>{accountingFormat(allInv)}</div></p>
								</div>
							</div>
						</div>
						<div class="col-xl-3 col-sm-6 col-12">
							<div class="card inovices-card" style={{height:'90%'}}>
								<div class="card-body">
									...
									<p class="inovices-all" style={{fontSize:'18px',display:'flex',justifyContent:'space-evenly',alignItems:'end',flexWrap:'wrap'}}>Paid Debit Note <span style={{fontSize:'15px'}}>{accountingFormat(paidInv)}</span></p>
								</div>
							</div>
						</div>
						<div class="col-xl-3 col-sm-6 col-12">
							<div class="card inovices-card" style={{height:'90%'}}>
								<div class="card-body">
									...
									<p class="inovices-all" style={{fontSize:'18px',display:'flex',justifyContent:'space-evenly',alignItems:'end',flexWrap:'wrap'}}>Unpaid Debit Note <span style={{fontSize:'15px'}}>{accountingFormat(unPaidInv)}</span></p>
								</div>
							</div>
						</div>
						<div class="col-xl-3 col-sm-6 col-12">
							<div class="card inovices-card" style={{height:'90%'}}>
								<div class="card-body">
									...
									<p class="inovices-all" style={{fontSize:'16px',display:'flex',justifyContent:'space-around',alignItems:'end',flexWrap:'wrap'}}>Cancelled Debit Note <span style={{fontSize:'14px'}}>{accountingFormat(canInv)}</span></p>
								</div>
							</div>
						</div>
					</div>
