...
const exportToExcel = async () => {
    ...
    useEffect(() => {
		...
		var url=new URL(window.location.href);
		  ...
		  let cgst = 0;
		  let sgst = 0;
        axios.post(`${process.env.REACT_APP_LOCAL_URL}/materialInward/${month1}`,{financialYear:localStorage.getItem("financialYear")},header).then((res) => {
	...
	// },3000)	
	  ...
	  function rendercommon(e,name , invt) {
		 ...
		 if(name == "Edit"){
			...
			console.log(e)
                debugger;
			axios.get(`${process.env.REACT_APP_LOCAL_URL}/cancelMaterialInward?QuoId=${invt}`,header).then((res)=>{
