...
const exportToExcel = async () => {
    ...
    useEffect(() => {
		...
		var url=new URL(window.location.href);
		  ...
		  let cgst = 0;
		  let sgst = 0;
        axios.post(`${process.env.REACT_APP_LOCAL_URL}/materialOutward/${month1}`,{financialYear:localStorage.getItem("financialYear")},header).then((res) => {
	  ...
	  function rendercommon(e,name , invt) {
		 ...
		 if(name == "Edit"){
			...
			if(res!=null && res.data.res=='sucess'){
				...
				//   alert("There is some issue delete invoice.");	
				  Swal.fire({
					icon: 'error',
					title: 'Oops...',
					text: 'There is some issue delete inventory.',
					footer: ''
				  })	   
		    });
		 } else if(name == "Cancel"){
			...
			// axios.post("http://localhost:8080/cancelInvoice/"+invt,{},header).then((res)=>{
                debugger;
			axios.get(`${process.env.REACT_APP_LOCAL_URL}/cancelMaterialOutward?QuoId=${invt}`,header).then((res)=>{
				if(res!=null && res.data.res=='success'){
					Swal.fire(
						'',
						'Inventory cancelled successfully!!',
						'success'
					  )	
					...
					Swal.fire({
						icon: 'error',
						title: 'Oops...',
						text: 'There is some issue to cancel inventory',
						footer: ''
					  })	
				}
			}).catch((e)=>{
									...
									<div class="col-lg-8 col-md-8">
										<div class="invoices-tabs">
											<ul>
												...
												<li><a href="#" class="active">All Material Outwards</a></li>
