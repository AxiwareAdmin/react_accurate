...
export default function ViewMaterialOutward() {
  ...
  useEffect(() => {
    ...
    if (!initilized.current) {
      ...
      axios
        .get(`${process.env.REACT_APP_LOCAL_URL}/viewMaterialOutward?invId=` + invId, header)
        .then((res) => {
  ...
  const sendMail=  (invoiceNo,custName) => {
    html2canvas(invoicepdf.current).then((canvas) => {
      ...
      axios.post(`${process.env.REACT_APP_LOCAL_URL}/sendmailMaterialOutward`, formData ,{
        headers: {
          Authorization: "Bearer " + token,
        },
      })
        ...
        .catch(error => {
          ...
          Swal.fire({
            icon: 'error',
            title: 'Oops...',
            text: 'There is some issue to mail inventory',
            footer: ''
            })
        });
    });
