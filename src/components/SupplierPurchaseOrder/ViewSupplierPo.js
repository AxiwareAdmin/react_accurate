...
export default function ViewSupplierPurchaseOrder() {
  ...
  useEffect(() => {
    ...
    if (!initilized.current) {
      ...
      axios
        .get(`${process.env.REACT_APP_LOCAL_URL}/viewSupplierPo?invId=` + invId, header)
        .then((res) => {
  ...
  const sendMail=  (invoiceNo,custName) => {
    html2canvas(invoicepdf.current).then((canvas) => {
      ...
      axios.post(`${process.env.REACT_APP_LOCAL_URL}/sendmailSupplierPo`, formData ,{
        headers: {
          Authorization: "Bearer " + token,
        },
      })
