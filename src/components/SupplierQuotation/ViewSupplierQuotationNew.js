...
(tempGstPercentageVal.length > 0
  ...
  useEffect(() => {
    ...
    if (!initilized.current) {
      ...
      axios
        .get(`${process.env.REACT_APP_LOCAL_URL}/${invoiceType==process.env.REACT_APP_CASH_SALE_INVOICE?'viewCashInvoice':invoiceType==process.env.REACT_APP_PROFORMA_INVOICE?'viewProformaInvoice':'viewSupplierQuotation'}?invId=` + invId, header)
        .then((res) => {
          debugger;
          ...
          res.data.invoiceProductDO.map((ele) => {
            ...
            if (index < 0) {
              tempGstPercentageArr = [...tempGstPercentageArr, roundNum(ele.tax)];
      ...
      axios.post(`${process.env.REACT_APP_LOCAL_URL}/sendmail`, formData ,{
        ...
        .catch(error => {
          ...
          Swal.fire({
            icon: 'error',
            title: 'Oops...',
            text: 'There is some issue to mail quotation',
            footer: ''
            })
        });
    });
